...
date
t1=`date +%s`
time aria2c --max-tries=32 --retry-wait=1 --async-dns=false -q -j 80 -x 16 -i icecube_in_list.txt
rc=$?
echo "RC: ${rc}"
t2=`date +%s`
let dt=$t2-$t1
...
t1=`date +%s`
for ((i=1; $i<${nmax}; i=$i+1)); do (cd subdir_$i; for ((j=0; $j<6; j=$j+1)); do aria2c --max-tries=32 --retry-wait=1 --async-dns=false -q --log-level=notice -l out.log -j 80 -x 16 -i ../icecube_in_list.txt; echo "RC: $?"; rm -f corsika* ; done ) & done
i=0
cd subdir_$i
  for ((j=0; $j<6; j=$j+1)); do 
    aria2c --max-tries=32 --retry-wait=1 --async-dns=false -q --log-level=notice -l out.log -j 80 -x 16 -i ../icecube_in_list.txt; 
    echo "RC: $?"; 
    fsize=`ls -l corsika* |awk '{a=a+$5}END{print a}'`
    rm -f corsika*
  done
