...
echo "Unifrac Unweighted Normalized"
...
for ((n=0; $n<153619; n=$n+19248)); do 
  ...
  let cstart=$a*16
  let cend=${cstart}+15
  (export ACC_DEVICE_NUM=$a; export ICORES=${cstart}-${cend}; export ISTART=$n; export IEND=$m; source /root/run_300k_wn_support.sh) &
  let a=$a+1
done 
...
for ((n=0; $n<153619; n=$n+19248)); do 
  ...
  let cstart=$a*16
  let cend=${cstart}+15
  (export ACC_DEVICE_NUM=$a; export ICORES=${cstart}-${cend}; export ISTART=${m1}; export IEND=$m; source /root/run_300k_wn_support.sh) &
  let a=$a+1
done 
wait
t2=`date +%s`
